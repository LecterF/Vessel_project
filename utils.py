...
class LogValidationCallback(pl.Callback):
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        image, label, out = outputs[0], outputs[1], outputs[2]
        img_digit = image.cpu().numpy()
        label_digit = label.squeeze(1).cpu().numpy()
        out_digit = out.argmax(axis=1).cpu().numpy()
        shape = image.shape
        mean = np.array(pl_module.hparams.mean_sen).reshape(1, -1, 1, 1)
        std = np.array(pl_module.hparams.std_sen).reshape(1, -1, 1, 1)
        ...
        trainer.logger.log_image('image', [Image.fromarray(img_digit[0, ...].transpose(1, 2, 0).astype(np.uint8))],
                              pl_module.current_epoch)
        label_digit = (label_digit * 255).astype(np.uint8)
        out_digit = (out_digit * 255).astype(np.uint8)
        trainer.logger.log_image('label', [Image.fromarray(label_digit[0, ...])], pl_module.current_epoch)
        trainer.logger.log_image('out', [Image.fromarray(out_digit[0, ...])], pl_module.current_epoch)
