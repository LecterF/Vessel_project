...
from metrics.corinf_metrics import CorInf
class MeInterface(pl.LightningModule):
    def __init__(self, metric_names, **kwargs):
        ...
        for metric_name in self.metric_names:
            ...
            if metric_name.lower() == 'corinf':
                self.metric_functions['corinf'] = CorInf()
    ...
    def calc_metrics(self, preds, labels, roi=None):
        ...
        for name, metric in self.metric_functions.items():
            if name != 'skeletal_similarity' and name != 'calscore' and name != 'corinf':
        ...
        for metric_name in self.metric_names:
            ...
            elif metric_name.lower() == 'cofinf':
                inf, _, cor = self.metric_functions['corinf'].compute_batch(preds, labels)
                results['cor'] = cor
                results['inf'] = inf
