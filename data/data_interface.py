...
class DInterface(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        if stage == 'fit' or stage is None:
            self.trainset = self.instancialize(train=True, data_dir=self.kwargs['train_dir'])
            self.valset = self.instancialize(train=False, data_dir=self.kwargs['val_dir'])

        # Assign test dataset for use in dataloader(s)
        if stage == 'test' or stage is None:
            self.testset = self.instancialize(train=False, data_dir=self.kwargs['test_dir'])
